...
UsersChatFrame = tk.Frame(BackGroundFrame,bg="light blue")#create a frame for the send messages
UsersChatFrame.grid_forget()
...
#button
MessageSender = tk.Button(SendMessageFrame,width=20,relief=tk.RAISED,bg="light blue",command=printMessage, image = img_messageSender)#size 20 x 20
MessageSender.pack(side=LEFT)

CreateChatButton = tk.Button(UserFunctionFrame1,relief=tk.RAISED,bg="light blue", text = "Create Chat",command=create)#makes you create a new chat
CreateChatButton.pack(fill = tk.X, side=LEFT)

LeaveChatButton = tk.Button(ChatNameFrame,relief=tk.RAISED,bg="light blue",text = "Leave Chat",command=leave)#makes you leave a new chat
LeaveChatButton.pack(fill = tk.X, side=RIGHT)

JoinChatButton = tk.Button(UserFunctionFrame1,relief=tk.RAISED,bg="light blue",text = "Join Chat",command=join)#makes you join a new chat
JoinChatButton.pack(fill = tk.X, side=LEFT)

SendButton = tk.Button(UserFunctionFrame1,relief=tk.RAISED,bg="cyan",text = "Submit",command=sent)#makes you send what you put 
SendButton.pack(fill = tk.X, side=BOTTOM)

OpenInfoMenu = tk.Button(ChatNameFrame,relief=tk.RAISED,bg="light blue",text = "Users",command=InfoChatUsers)#makes you send what you put 
OpenInfoMenu.pack(fill = tk.X, side=RIGHT)
...
for i,name in enumerate(users):
    user = tk.Label(UsersChatFrame,text=name,bg="light blue",fg="white",relief=tk.FLAT)#puts the username in the frame
    user.pack(side = TOP)
